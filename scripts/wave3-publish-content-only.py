#!/usr/bin/env python3
"""
wave3-publish-content-only.py — Publish Wave-3 rebuilt content to 5 EV pages.
...
Usage: python3 wave3-publish-content-only.py
"""
...
# Pages: (draft_file_relative_to_CORE30, wp_id)
PAGES = [
    ("13-panel-upgrade-mclean-va/draft-v2-WP-WRAPPED.html", 6290),
    ("15-ev-charger-mclean-va/draft-v2-WP-WRAPPED.html", 6330),
    ("07-electrical-troubleshooting-mclean-va/draft-v3-WP-WRAPPED.html", 6260),
    ("17-panel-upgrade-oakton-va/draft-v2-WP-WRAPPED.html", 6292),
    ("23-panel-upgrade-tysons-va/draft-v2-WP-WRAPPED.html", 6333),
]
...
def main():
    ...
    headers = {
        ...
        "User-Agent": "wave3-publish/1.0",
    }

    # ---- Step 1: Verify all drafts exist ----
    resolved = []
    for draft_rel, wp_id in PAGES:
        draft_path = CORE30 / draft_rel
        if not draft_path.exists():
            print(f"FATAL: draft not found: {draft_path}")
            return 1
        slug = draft_rel.split("/")[0].split("-", 1)[1]
        resolved.append((draft_path, wp_id, slug))

    print(f"Wave-3 pages to publish ({len(resolved)}):")
    for _, wp_id, slug in resolved:
        print(f"  {slug} → WP {wp_id}")
    ...
    # ---- Step 4: Verify each page live (cache-busted) ----
    print("\nVerifying live pages (cache-busted)...")
    all_pass = True
    for slug, wp_id, published, _ in results:
        ...
        try:
            resp = requests.get(
                live_url,
                timeout=30,
                headers={"User-Agent": "wave3-verify/1.0", "Cache-Control": "no-cache"},
            )
        except Exception as e:
        ...
        # Check for new Wave-3 content markers
        has_evp_corepage = "evp-corepage" in body
        has_heading_left = "evp-heading-left" in body

        # Check images
        img_srcs = re.findall(r'<img[^>]+src="([^"]+)"', body)
        ev_imgs = [s for s in img_srcs if "evelectric.pro" in s]
        img_results = []
        for img_url in ev_imgs[:10]:
            try:
        ...
        # Check no goldenservices leak
        golden_leak = "goldenservices" in body.lower()

        print(f"\n  {slug} (WP {wp_id}):")
        print(f"    HTTP 200: {'YES' if status_ok else 'NO (' + str(resp.status_code) + ')'}")
        print(f"    Size: {size_kb:.0f} KB")
        print(f"    New content (evp-corepage): {'YES' if has_evp_corepage else 'NO'}")
        print(f"    heading-left rule: {'YES' if has_heading_left else 'NO'}")
        print(f"    Images ({len(ev_imgs)} found, all 200): {'YES' if imgs_ok else 'NO — ' + str(img_fails)}")
        print(f"    Dollar figures: {'NONE' if not dollar_matches else 'FOUND: ' + str(dollar_matches)}")
        print(f"    goldenservices leak: {'NONE' if not golden_leak else 'LEAK FOUND'}")

        if not (status_ok and has_evp_corepage and imgs_ok and not golden_leak):
            all_pass = False
